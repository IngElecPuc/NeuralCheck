...
class TheoryWindow:
    ...
    def _build_layout(self) -> None:
        ...
        tk.Label(self.main_frame, textvariable=self.status_var, anchor="w").grid(row=2, column=0, sticky="ew", padx=4, pady=(0, 2))

        self.map_frame = tk.LabelFrame(self.main_frame, text="Mapa visual")
        self.map_frame.grid(row=3, column=0, sticky="nsew", padx=4, pady=4)
        ...
        self.theory_map = TheoryMapCanvas(
            ...
            on_node_edit_requested=self.open_node_edit_popup,
    ...
    def save_selected_node(self) -> None:
        ...
        if node is not None:
            self.open_node_edit_popup(node.id)
    ...
    def open_node_edit_popup(self, node_id: str) -> None:
        node = self.controller.service.get_node(node_id)
        if node is None:
            return
        ...
        tk.Label(popup, text="Nombre:").grid(row=0, column=0, sticky="w", padx=6, pady=4)
        name_entry = tk.Entry(popup, width=42)
        name_entry.grid(row=0, column=1, sticky="ew", padx=6, pady=4)
        if node.name:
            name_entry.insert(0, node.name)

        tk.Label(popup, text="Evaluación:").grid(row=1, column=0, sticky="w", padx=6, pady=4)
        eval_entry = tk.Entry(popup, width=20)
        eval_entry.grid(row=1, column=1, sticky="ew", padx=6, pady=4)
        if node.evaluation:
            eval_entry.insert(0, node.evaluation)

        readonly = tk.Frame(popup)
        readonly.grid(row=2, column=0, columnspan=2, sticky="ew", padx=6, pady=4)
        readonly.columnconfigure(1, weight=1)
        tk.Label(readonly, text="Capturadas:").grid(row=0, column=0, sticky="nw", padx=(0, 6), pady=2)
        tk.Label(readonly, text=node.captured_pieces or "—", anchor="w").grid(row=0, column=1, sticky="ew", pady=2)
        tk.Label(readonly, text="Turno:").grid(row=1, column=0, sticky="nw", padx=(0, 6), pady=2)
        tk.Label(readonly, text=node.side_to_move, anchor="w").grid(row=1, column=1, sticky="ew", pady=2)
        tk.Label(readonly, text="FEN:").grid(row=2, column=0, sticky="nw", padx=(0, 6), pady=2)
        fen_text = tk.Text(readonly, width=68, height=3, wrap=tk.WORD)
        fen_text.grid(row=2, column=1, sticky="ew", pady=2)
        fen_text.insert("1.0", node.fen)
        fen_text.config(state="disabled")

        warning_var = tk.StringVar(value="")
        warning_label = tk.Label(popup, textvariable=warning_var, foreground="#991b1b", wraplength=520, justify="left")
        warning_label.grid(row=3, column=0, columnspan=2, sticky="ew", padx=6, pady=(2, 0))

        buttons = tk.Frame(popup)
        buttons.grid(row=4, column=0, columnspan=2, sticky="e", padx=6, pady=8)
        delete_confirmation = {"armed": False}
        ...
        def accept() -> None:
            try:
                ...
                self.controller.update_selected_node(
                    ...
                    evaluation=eval_entry.get().strip() or None,
                    captured_pieces=node.captured_pieces,
                )
            except Exception as exc:
                messagebox.showerror("No se pudo guardar el nodo", str(exc), parent=popup)
                return
            popup.destroy()
            self.refresh_all()

        def delete_or_arm() -> None:
            if not delete_confirmation["armed"]:
                try:
                    preview = self.controller.preview_delete_subtree(node_id)
                except Exception as exc:
                    messagebox.showerror("No se pudo calcular el borrado", str(exc), parent=popup)
                    return
                warning_var.set(
                    f"Se borrarán {preview.node_count} nodo(s) y {preview.edge_count} relación(es). "
                    "Presiona Confirmar eliminar para completar."
                )
                delete_confirmation["armed"] = True
                delete_button.config(text="Confirmar eliminar")
                return
            parent_before_delete = self.controller.get_parent_branch(node_id)
            try:
                self.controller.delete_subtree(node_id)
            except Exception as exc:
                messagebox.showerror("No se pudo eliminar el nodo", str(exc), parent=popup)
                return
            if parent_before_delete is not None:
                self.controller.select_node(parent_before_delete.node.id)
            elif self.controller.selected_book_id is not None:
        ...
        tk.Button(buttons, text="Aceptar", command=accept).grid(row=0, column=0, padx=3)
        delete_button = tk.Button(buttons, text="Eliminar", command=delete_or_arm)
        delete_button.grid(row=0, column=1, padx=3)
        tk.Button(buttons, text="Cancelar", command=popup.destroy).grid(row=0, column=2, padx=3)
